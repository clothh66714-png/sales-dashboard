...
async function fetchAllocProject(token, projectId, nameMap) {
  ...
  for (const sec of trackingSections) {
    const url = `https://app.asana.com/api/1.0/sections/${sec.gid}/tasks?opt_fields=name,assignee.name,modified_at,created_at,completed&limit=100`;
    const res = await fetchJson(url, { Authorization: `Bearer ${token}` });
    const tasks = (res.data || [])
      .filter(t => !t.completed)  // 排除已完成的任務
      .map(t => ({ ...t, sectionName: sec.name }));
    allTasks.push(...tasks);
  ...
  Object.entries(merged).forEach(([name, data]) => {
    ...
    const overdueList = data.overdue
      ...
      .map(t => ({
        ...
        days: t.daysSince,
        status: t.section,
        url: `https://app.asana.com/0/${t.gid}/${t.gid}`,
      }));
